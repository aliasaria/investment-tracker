...
const SKIP_ACTIVITIES = new Set(["DISTRIB."]);

const DATE_PREFIX_RE = /^([A-Z]{3})\.\s*(\d{1,2})\s+(.*)$/;
...
function parseActivity(text, asOfDate) {
  ...
  for (let i = startIdx + 1; i < lines.length; i++) {
    ...
    if (/^[A-Z]{3}\.\s+\d{1,2}\s*$/.test(line)) continue; // date line on its own (header date)
