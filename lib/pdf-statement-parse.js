...
// The CASH BALANCE section sits between the asset summary and the asset review.
// It lists per-account-type opening + closing balances. Format:
//   Cash $45,814.89 $89,054.88
//   Other $33.84 $164.54
//   Margin - Long $160.56 $160.56
// The closing balance (second $amount) is what we want to capture as a holding —
// these balances are NOT included in the ASSET REVIEW section, so without this
// parser the portfolio total is understated by the cash position.
const CASH_ROW_RE = /^([A-Za-z][A-Za-z0-9 ./&\-]*?)\s+\$([\d,]+\.\d{2})\s+\$([\d,]+\.\d{2})$/;

function parseCashBalance(text) {
  const lines = text.split("\n");
  const startIdx = lines.findIndex((l) => l.trim() === "CASH BALANCE");
  if (startIdx === -1) return [];
  const rows = [];
  for (let i = startIdx + 1; i < Math.min(startIdx + 30, lines.length); i++) {
    const line = lines[i].trim();
    if (!line) continue;
    if (line === "ASSET REVIEW" || line.startsWith("ASSET REVIEW")) break;
    if (line === "ACCOUNT ACTIVITY") break;
    const m = line.match(CASH_ROW_RE);
    if (!m) continue;
    const type = m[1].trim();
    const closing = parseFloat(m[3].replace(/,/g, ""));
    if (closing === 0) continue; // don't emit zero-balance holdings
...
function parseStatement({ currency, text }) {
  ...
  const securities = parseHoldings(text);
  const cashRows = parseCashBalance(text).map((c) => ({
    symbol: null,
    name: c.type,
    productType: "CASH",
    totalValue: c.closing,
  }));
  const holdings = [...cashRows, ...securities];
  const activity = parseActivity(text, asOfDate);

  return { accountNumber, asOfDate, currency, fxRate, holdings, activity };
}

module.exports = { parseStatement, parseHoldings, parseHoldingRow, parseCashBalance };
