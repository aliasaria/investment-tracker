...
async function ingestActivity({ rows, uploadTimestamp, db, yahooFinance }) {
  const insertStmt = db.prepare(`
    INSERT OR IGNORE INTO cash_flows
      (date, account_name, amount_cad, amount_original, currency_original,
  ...
  for (const row of rows) {
    ...
    if (!date || !account || isNaN(amount) || !(currency === "CAD" || currency === "USD")) {
      skipped++;
      continue;
    }
    ...
    prepared.push({
      date, account, amountCad, amount, currency, fxRate, activity, description,
  ...
  const insertMany = db.transaction((preps) => {
    for (const p of preps) {
      const info = insertStmt.run(
        p.date, p.account, p.amountCad, p.amount, p.currency,
